"""
Management command to fix migration state inconsistencies in HRM service.
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with connection.cursor() as cursor:
            # Check if drafted_at column exists in payroll_payrollrun
            cursor.execute("""
                SELECT column_name 
            ...
            self.stdout.write(f"Existing columns in payroll_payrollrun: {existing_columns}")
            ...
            required_columns = ['drafted_at', 'posted_at', 'posted_by']
